...
public class ServerSocketService {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					String ip = socketChannel.getRemoteAddress()
							.toString(); /*
	...
	public void serverStop() {
		...
		if (!SysInfo.users.isEmpty()) {
			...
			kStream.forEach(userSocket -> {
				try {
					userSocket.socketChannel.close();
				} catch (IOException e) {}
			});
			SysInfo.users.clear();
		}
		...
		if (SysInfo.serverSocketChannel.isOpen()) {
			try {
				SysInfo.serverSocketChannel.close();
			} catch (IOException e) {}
		}
		/** ThreadPool **/
	...
	public static void banId(String banId) {
		Thread t = new Thread() {
			@Override
			public void run() {
				MultiChatRunnable m;
				if ((m = Finder.socket(banId)) != null) {
					try {	
						System.out.println(SysInfo.users.get(m).getCid());
						Packet packet = new Packet();
						packet.setProtocol(Protocol.BANNED_ID);
						
						m.socketChannel.close();
						SysInfo.users.remove(m);
					} catch (IOException e) {}
				}
			};
		};
		SysInfo.threadPoolExecutor.submit(t);
	}
}
